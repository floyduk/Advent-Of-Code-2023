...
def costs(state):
    ...
    if dir != None:
        nx = x + dxdy[dir][0]
        ny = y + dxdy[dir][1]
        if is_in_bounds(nx, ny) and count+1 <= 10:
            possible_edges.append((input[ny][nx], ((nx,ny), dir, count+1))) 

    # Try a turn
    for nd in available_directions[dir]:
        # move forward 4 steps after any turn
        nx = x + (4 * dxdy[nd][0])
        ny = y + (4 * dxdy[nd][1])

        # If the destination is in bounds then calculate the cost to that destination
        if is_in_bounds(nx, ny):
            if nd == 'w':
                total_cost = input[y][x-1] + input[y][x-2] + input[y][x-3] + input[y][x-4]
            elif nd == 'e':
                total_cost = input[y][x+1] + input[y][x+2] + input[y][x+3] + input[y][x+4]
            elif nd == 's':
                total_cost = input[y+1][x] + input[y+2][x] + input[y+3][x] + input[y+4][x]
            elif nd == 'n':
                total_cost = input[y-1][x] + input[y-2][x] + input[y-3][x] + input[y-4][x]
            
            # Add an edge to this new destination with the combined cost
            possible_edges.append((total_cost, ((nx,ny), nd, 4)))
